...
public class Discotienda
{
   ...
   /**
    * Generar un informe de todos los discos Pop, Rock y que ademas sean económicos. (No superar 1000 mil pesos) 
    * @throws FileNotFoundException - cuando no sse encuentre discos con esas condiciones
    */
   
   public void generarInformeDiscosPopRockCostosos()throws FileNotFoundException{
	   
	 // crear el archivo discosCostosos con la clase FILE
	   File archivoPopRockC = new File("./data/discosCostosos.txt");
	   
	 // crear pluma para escribir el archivo 
  		PrintWriter plumaRPC = new PrintWriter(archivoPopRockC);
  		
  	  	//escribir con la pluma en el archivo 
  		plumaRPC.println("Reporte de discos Pop Rock costosos");
  		plumaRPC.println("====================================");

  		//escribir el reporte de los discos costosos 
  		//=======================================================
  		
  		//recorrido total de los discos 
  		for(int i=0; i < discos.size(); i++) {
  			//extraer infromacion de cada disco
  			
  			Disco miDisco=(Disco)discos.get(i);
  			
  			//sentencia if para comparar el precio de los discos
  			if(miDisco.darPrecioDisco()>1000) {
  				plumaRPC.println("El precio del disco es mayor que 1000");
  				plumaRPC.println("Precio" + miDisco.darPrecioDisco());
  			}
    ...
    {
        try{
        	generarInformeDiscosPopRockCostosos();
